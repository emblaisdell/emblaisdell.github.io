...
export const OPENING = [
  {
    ...
    body: 'Your shop mints NAND gates and nothing else. Your first client wants inverters, so you will have to design one. Open the RECORDING BENCH.',
  ...
  },
  {
    ...
    body: 'Press RECORD. The stopwatch runs until you press DONE, and that time becomes the cycle time of the process forever — so it pays to know what you are building before you start.',
    target: '#btnRecord',
  ...
  },
  {
    id: 'place',
    title: 'Place a NAND',
    body: 'Click NAND in the library on the left, then click the sheet to drop one on the bench. Copies are free while recording.',
    target: '#palette',
  ...
  },
  {
    ...
    body: 'The panel matched your truth table against the catalogue and calls it NOT. Press DONE to stop the clock and add it to your library.',
    target: '#btnDone',
  ...
  },
  {
    ...
    body: 'Nothing had to be designated: your NOT behaves like what INVERSE HOLDINGS ordered, so the SHIP row at the bottom of the schedule picked it up. Drag that row above a process and it would take the circuits that process needs — which is why shipments sit at the bottom by default.',
    target: '#clients',
  ...
  },
  {
    ...
    body: 'Record a circuit, run it as a process, and the schedule ships it. Recorded circuits are ingredients for the next recording — AND is one NAND plus one NOT — and when two rows want the same circuit, the higher one draws first. When a client wants something you cannot work out, pay for a testbench run in the order book: they will tell you exactly which inputs your circuit gets wrong.',
    last: true,
...
export const MEMORY = [
  {
    ...
    body: 'Everything so far has been a function: same inputs, same output, every time. This order wants a part that holds a bit — and the only way to build one is to let a wire run backwards, so the circuit can see what it did last. Open the RECORDING BENCH.',
    target: '[data-screen=record]',
    done: ({ state }) => state.screen === 'record',
  },
  {
