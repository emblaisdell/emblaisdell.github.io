...
When I first learned about Gödel's First Incompleteness Theorem, probably in early undergrad, I was fascinated by the idea of a provably unprovable and irrefutable claim.  This claim is true or false in the 'real world' ($\Th(\mathbb{N})$), so there is a true claim we cannot prove.
...
- introduces Löb's theorem for PA,
- shows how Löb's theorem is induction for Kripke frames (i.e. implies finiteness) and
- jokingly shows how Löb's theorem is humility in doxastic logic,
- then introduces the axiomatization of Gödel-Löb,
